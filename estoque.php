...
if ($search_term) {
    ...
    $search_query = "WHERE produto LIKE '%$search_term%' OR quantidade LIKE '%$search_term%' OR unidade LIKE '%$search_term%'";
}

$sql = "SELECT * FROM produtos $search_query";
...
<head>
    ...
    <style>
        ...
        .low-quantity {
    background-color: #f8d7da !important; /* Cor de fundo vermelha */
    color: #721c24 !important; /* Cor do texto para contraste */
}
...
<body>
    ...
    <nav class="w3-sidebar w3-collapse w3-top w3-large" style="z-index:3;width:300px;font-weight:bold;background-color:#eeeeee;color:#fff;" id="mySidebar">
        ...
        <div class="w3-bar-block" style="font-size:23px;">
            ...
            <a href="estoque.php" onclick="w3_close()" class="w3-bar-item w3-red w3-button w3-hover-red">Estoque</a>
...
<a href="logout.php" style="position: fixed; bottom: 20px; left: 20px; font-size: 18px; display: flex; align-items: center; justify-content: center; text-decoration: none; color: black; padding: 10px 15px;">
    ...
    <div class="w3-main" style="margin-left:340px;margin-right:40px;">
        ...
        <div class="w3-container" style="margin-top:80px" id="showcase">
            <h1 class="w3-jumbo"><b>Controle de Estoque</b></h1>
                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr" . (intval($row['quantidade']) <= 3 ? " class='low-quantity'" : "") . ">";
                            echo "<td>" . htmlspecialchars($row['produto']) . "</td>";
                            echo "<td>" . htmlspecialchars($row['quantidade']) . "</td>";
                            echo "<td>" . htmlspecialchars($row['unidade']) . "</td>";
                            echo '<td><a href="edit.php?id=' . $row['id'] . '" class="edit-btn"><i class="fas fa-pencil-alt"></i></a>';
                            echo '<a href="delete.php?id=' . $row['id'] . '" class="delete-btn"><i class="fas fa-trash"></i></a></td>';
                            echo "</tr>";
                        }
                    } else {
        ...
        <!-- Add Product Button -->
        <button class="add-product-btn" onclick="toggleForm()">
            <i class="fas fa-plus"></i>
        </button>

        <!-- Add Product Form -->
        <div id="addProductForm" class="form-container" style="display: none; margin-top: 20px;">
